...
"#, output_dir, file_name)
    ...
    /// Generate a wrapper script for tracemalloc
    fn generate_tracemalloc_wrapper(&self, file_name: &str, output_dir: &str) -> String {
        format!(r#"
#!/usr/bin/env python
...
# Try to import tracemalloc
try:
    import tracemalloc
except ImportError:
    print("Warning: tracemalloc not available in this Python version")
    sys.exit(1)

# Set up output files
snapshot_file = "{}/tracemalloc_snapshot.txt"
lifetime_file = "{}/object_lifetime.txt"
...
# Force garbage collection to clean up any unused objects
gc.collect()

# Take snapshot and write results
...
with open(snapshot_file, "w") as f:
    ...
    for i, stat in enumerate(top_stats[:100], 1):
        f.write(f"#{{i}}: {{stat.size_kb:.1f}} KB - {{stat.count}} blocks: {{stat.traceback.format()[0]}}\n")
        for frame in stat.traceback.format()[1:10]:  # Show up to 10 frames
            f.write(f"    {{frame}}\n")
    
    # Group by file and line
    f.write("\n\nMemory allocations by file:\n")
    file_stats = snapshot.statistics("filename")
    for stat in file_stats:
        f.write(f"{{stat.size_kb:.1f}} KB - {{stat.count}} blocks: {{stat.traceback.format()[0]}}\n")

    # Group by category
    f.write("\n\nMemory allocations by category:\n")
    import re
    categories = {{"str": 0, "list": 0, "dict": 0, "set": 0, "tuple": 0, "function": 0, "other": 0}}
    category_counts = {{"str": 0, "list": 0, "dict": 0, "set": 0, "tuple": 0, "function": 0, "other": 0}}
    
    for stat in snapshot.statistics("traceback"):
        tb_str = str(stat.traceback)
        if "str" in tb_str:
            categories["str"] += stat.size
            category_counts["str"] += stat.count
        elif "list" in tb_str:
            categories["list"] += stat.size
            category_counts["list"] += stat.count
        elif "dict" in tb_str:
            categories["dict"] += stat.size
            category_counts["dict"] += stat.count
        elif "set" in tb_str:
            categories["set"] += stat.size
            category_counts["set"] += stat.count
        elif "tuple" in tb_str:
            categories["tuple"] += stat.size
            category_counts["tuple"] += stat.count
        elif "function" in tb_str:
            categories["function"] += stat.size
            category_counts["function"] += stat.count
        else:
            categories["other"] += stat.size
            category_counts["other"] += stat.count
    
    for category, size in categories.items():
        count = category_counts[category]
        f.write(f"{{category}}: {{size / 1024:.1f}} KB in {{count}} objects\n")

# Object lifetime tracking and circular reference detection
try:
    import objgraph
    with open(lifetime_file, "w") as f:
        f.write("Object Lifetime and Reference Analysis:\n\n")
        
        # Check for common Python objects
        f.write("Most common object types:\n")
        for obj_type, count in objgraph.most_common_types(limit=20):
            f.write(f"{{obj_type}}: {{count}} instances\n")
        
        # Check for potential memory leaks (objects that should have been garbage collected)
        f.write("\nPotential memory leaks (objects that should have been garbage collected):\n")
        try:
            # Search for objects that might be leaking
            for obj_type in ["list", "dict", "set", "tuple"]:
                objects = objgraph.by_type(obj_type)
                if len(objects) > 100:  # Arbitrary threshold
                    f.write(f"Warning: Large number of {{obj_type}} objects: {{len(objects)}}\n")
                    
                    # Sample some objects and show their reference paths
                    for obj in objects[:5]:  # Show first 5
                        f.write(f"Sample {{obj_type}} reference paths:\n")
                        for chain in objgraph.find_backref_chain(obj, objgraph.is_proper_module, max_depth=10):
                            f.write(f"  {{repr(chain)}}\n")
        except Exception as e:
            f.write(f"Error in memory leak detection: {{e}}\n")
        
        # Check for circular references
        f.write("\nPotential circular references:\n")
        try:
            # Force collection to make circular references more obvious
            gc.collect()
            
            # Get objects that are still in memory
            gc_objects = gc.get_objects()
            circular_candidates = [o for o in gc_objects if isinstance(o, (list, dict, set)) and len(o) > 0]
            
            # Check a sample for circular references
            found_circular = False
            for obj in circular_candidates[:100]:  # Check first 100 candidates
                try:
                    chain = objgraph.find_ref_cycle(obj, max_depth=5)
                    if chain:
                        found_circular = True
                        f.write(f"Found circular reference: {{repr(chain)}}\n")
                        if len(chain) <= 3:  # Only show details for simple cycles
                            f.write(f"Details: {{chain}}\n")
                except Exception as e:
                    pass  # Skip objects that can't be analyzed
                    
            if not found_circular:
                f.write("No obvious circular references detected in sample.\n")
        except Exception as e:
            f.write(f"Error in circular reference detection: {{e}}\n")
except ImportError:
    with open(lifetime_file, "w") as f:
        f.write("Install objgraph for object lifetime and circular reference detection.\n")
        f.write("pip install objgraph\n")
