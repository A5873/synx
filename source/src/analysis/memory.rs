    ...
    fn identify_memory_issues(&self, memory_details: &MemoryDetails) -> Vec<AnalysisIssue> {
        ...
        for leak in &memory_details.leaks {
            ...
            issues.push(AnalysisIssue {
                ...
                title: format!("Memory leak of {} bytes", leak.size),
                analysis_type: AnalysisType::Memory,
                locations: vec![leak.allocation_location.clone()],
            });
        }
        ...
        if memory_details.total_allocations > 0 && 
           memory_details.total_deallocations == 0 {
            issues.push(AnalysisIssue {
                ...
                title: "Systematic memory leaks detected".to_string(),
                analysis_type: AnalysisType::Memory,
                locations: Vec::new(),
            });
        }
        ...
        if !memory_details.allocation_hotspots.is_empty() {
            ...
            if *bytes > 1_000_000 {
                issues.push(AnalysisIssue {
                    ...
                    title: format!("Memory allocation hotspot in '{}'", hotspot_fn),
                    analysis_type: AnalysisType::Memory,
                    locations: vec![hotspot_fn.clone()],
                });
            }
        }
...
fn find_xml_tag<'a>(content: &'a str, tag_name: &str) -> Option<&'a str> {
...
fn find_xml_tag_after<'a>(content: &'a str, tag_name: &str, after: &'a str) -> Option<&'a str> {
...
fn extract_xml_content<'a>(tag: &'a str, tag_name: &str) -> String {
    let start_tag = format!("<{}>", tag_name);
    let end_tag = format!("</{}>", tag_name);
    ...
    fn parse_memory_errors(&self, asan_output: &str) -> Vec<AnalysisIssue> {
        ...
        if let Some(pos) = asan_output.find("ERROR: AddressSanitizer: heap-use-after-free") {
            ...
            if let Some(error_end) = asan_output[error_start..].find("\n\n") {
                ...
                issues.push(AnalysisIssue {
                    title: "Use After Free Detected".to_string(),
        ...
        if let Some(pos) = asan_output.find("ERROR: AddressSanitizer: heap-buffer-overflow") {
            ...
            if let Some(error_end) = asan_output[error_start..].find("\n\n") {
                ...
                issues.push(AnalysisIssue {
                    title: "Heap Buffer Overflow".to_string(),
                    severity: IssueSeverity::Critical,
        ...
        if let Some(pos) = asan_output.find("ERROR: AddressSanitizer: stack-use-after-return") {
            ...
            if let Some(error_end) = asan_output[error_start..].find("\n\n") {
                ...
                issues.push(AnalysisIssue {
                    title: "Stack Use After Return".to_string(),
                    severity: IssueSeverity::Critical,
        ...
        if let Some(pos) = asan_output.find("ERROR: AddressSanitizer: global-buffer-overflow") {
            ...
            if let Some(error_end) = asan_output[error_start..].find("\n\n") {
                ...
                issues.push(AnalysisIssue {
                    title: "Global Buffer Overflow".to_string(),
                    severity: IssueSeverity::Critical,
...
impl RustMemoryLeakDetector {
    ...
    fn parse_safety_issues(&self, safety_output: &str) -> Vec<AnalysisIssue> {
        ...
        if safety_output.contains("error: Undefined Behavior") {
            ...
            issues.push(AnalysisIssue {
                title: format!("Memory safety issue: {}", if error_msg.contains("dangling reference") {
                    "Dangling Reference"
                } else if error_msg.contains("memory leak") {
                    "Memory Leak"
                } else {
    ...
    fn identify_memory_issues(&self, memory_details: &MemoryDetails) -> Vec<AnalysisIssue> {
        ...
        if !memory_details.allocation_hotspots.is_empty() {
            ...
            if *bytes > 1_000_000 {
            issues.push(AnalysisIssue {
                title: format!("Memory Hotspot: {}", hotspot_fn),
                severity: IssueSeverity::Medium,
                description: format!(
                    "Memory allocation hotspot: function '{}' allocated {} bytes ({}MB)",
                    hotspot_fn, bytes, bytes / 1_000_000
...
impl PythonMemoryAnalyzer {
    /// Generate a tracemalloc wrapper script for memory profiling
    fn generate_tracemalloc_wrapper(&self, file_name: &str, output_dir: &Path) -> Result<String> {
        Ok(format!(r#"
import tracemalloc
import sys
import os
...
file_path = "{}"
output_file = str(Path("{}") / "tracemalloc_snapshot.txt")
...
try:
    ...
    # Take snapshot and get statistics
    snapshot = tracemalloc.take_snapshot()
    top_stats = snapshot.statistics('lineno')
    
    # Write results
    with open(output_file, 'w') as f:
        f.write("Memory Trace Results:\n\n")
        for stat in top_stats:
            frame = stat.traceback[0]
            f.write(f"{{stat.size}} bytes allocated at {{frame.filename}}:{{frame.lineno}}\n")
        ...
        for block in snapshot.traces_by_lineno:
            f.write(f"Size: {{block.size}} bytes\n")
            for frame in block.traceback:
                f.write(f"    {{frame.filename}}:{{frame.lineno}}\n")

except Exception as e:
    print(f"Error in tracemalloc analysis: {{e}}", file=sys.stderr)
    sys.exit(1)
"#, file_name, output_dir.display()))
    }
...
"##, output_dir, file_name)
    ...
    fn identify_memory_issues(&self, memory_details: &MemoryDetails) -> Vec<AnalysisIssue> {
        ...
        for leak in &memory_details.leaks {
            ...
            issues.push(AnalysisIssue {
                title: format!("Python memory issue: {}", if leak.allocation_location.contains("Circular reference") {
                    "Circular Reference"
                } else {
                    "Memory Leak"
                }),
        ...
        if !memory_details.allocation_hotspots.is_empty() {
            ...
            for (i, (hotspot, bytes)) in top_hotspots.enumerate() {
                if *bytes > 1_000_000 {
                    issues.push(AnalysisIssue {
                        title: format!("Memory Hotspot #{}", i+1),
                        severity: IssueSeverity::Medium,
                        description: format!(
        ...
        if memory_details.heap_usage_timeline.len() > 2 {
            ...
            if consistently_increasing && last > first * 2 {
                issues.push(AnalysisIssue {
                    title: "Memory Leak: Growing Memory Usage".to_string(),
                    severity: IssueSeverity::High,
                    description: format!(
